...
ir_token_t* new_token(enum ID id, char* literal, int length, int line, int column) {
...
char lexer_peek_next(ir_lexer_t* lex) {
    if (lexer_peek(lex) == '\0') {
        return '\0';
    }
...
ir_token_t* lexer_read_ident(ir_lexer_t* lex) {
    ...
    do {
        lexer_advance(lex);
        length++;
    } while(is_letter(lexer_peek(lex)));
...
ir_token_t* lexer_read_string(ir_lexer_t* lex) {
    ...
    lexer_advance(lex); // consumes open '"'
    int length = 0;
    char* start = lex->current;
    ...
    lexer_advance(lex); // consumes closing '"'
